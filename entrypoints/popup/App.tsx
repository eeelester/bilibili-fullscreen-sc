  ...
  useLayoutEffect(() => {
    void (async () => {
      ...
      if (!isMatch) {
        ...
        const tmp: boolean | null | undefined = await storage.getItem('session:switchState')
...
async function onSwitch(switchState: boolean) {
  ...
  // 存着，不然下次点击popup就没有了
  await storage.setItem<boolean>('session:switchState', switchState)

  // 发送给background，这个开关是用户操作，级别最高，background的根据url切换图标状态要在这之下
  await browser.runtime.sendMessage({ switchState })

  void changeIcon(switchState)
}
